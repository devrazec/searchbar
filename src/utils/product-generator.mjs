...
const sellerJson = new URL('../../src/data/seller-sorted.json', import.meta.url);
...
const rawProductJson = new URL('../../src/data/raw-product.json', import.meta.url);
const mongodbImageJson = new URL('../../src/data/mongodb-image.json', import.meta.url);
const productJson = new URL('../../src/data/product.json', import.meta.url);

// 1️⃣ Read all JSON files
const [sellers, brands, locations, genders, colors, categories, rawProducts, mongodbImages] =
  await Promise.all([
...
function getRandomLatLng(location) {
  ...
  const lat = Math.random() * (location.latMax - location.latMin) + location.latMin;
  const lng = Math.random() * (location.lngMax - location.lngMin) + location.lngMin;
  return { lat, lng };
}
